import { Family } from "../dto/dtos";

export class SPReasonSupplementer {
    familes: Array<Family>;
    constructor(familes: Array<Family>) {
        this.familes = familes;
    }
    supplement() {
        let alreadyTargets = Array<number>();
        // for user

        // 被保護世帯 welfareHouseholdFlag = 1                3
        this.familes[3].user.welfareHouseholdFlag = 1;
        this.familes[3].interviews.forEach(e => {e.welfareHouseholdFlag = 1;});
        // 非課税世帯 taxExemptHouseholdFlag = 1              4
        this.familes[4].user.taxExemptHouseholdFlag = 1;
        this.familes[4].interviews.forEach(e => {e.taxExemptHouseholdFlag = 1;});
        // ひとり親世帯 singleParentHouseholdFlag = 1         5
        this.familes[5].user.singleParentHouseholdFlag = 1;
        this.familes[5].interviews.forEach(e => {e.singleParentHouseholdFlag = 1;});
